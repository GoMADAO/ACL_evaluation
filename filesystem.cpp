...
void FileSystem::addACLFile(std::vector<std::string> *user_group,
  ...
  ACLFile* cur = new ACLFile(name,*user_group);
...
void FileSystem::showUserGroup(){
  ...
  for(std::map<std::string, std::set<std::string>> :: iterator it = users.begin(); it!=users.end();++it){
    user_name = it->first;
  ...
  for(std::map<std::string, std::set<std::string>> :: iterator it = groups.begin(); it!=groups.end();++it){
    group_name = it->first;
...
void showACLpair(std::vector<acl_pair> file_permission){
  for(size_t n=0; n<file_permission.size();++n)
    std::cout<<"user: "<<file_permission[n].first<<" permission:"<< file_permission[n].second <<std::endl;
}
...
void showFileHelper(ACLFile* p){
  ...
  for(std::map<std::string, ACLFile*> :: iterator it=p->file_link.begin(); it!=p->file_link.end();++it){
    showFileHelper(it->second);
  }
  
}

void FileSystem::showFilePermission(){
  showFileHelper(root);
}
